...
public class LAB05 {
	...
	public static WhyDontStructsExistInJava[] generateCriticalPaths(DirectedGraph g, String[] v, HashMap<String, Integer> weights) throws InputNotDAGException {
		...
		for (String vertex : zeroes) {
			finalweights[getIndexOf(vertex, vertices)] = 0;
		}
		...
		while(zeroes.size() > 0) {
			...
			for (String vertex : zeroes) {
				...
				indegrees[previx]--; // sets idegrees to -1 (sentinel value meaning this is not part of the working graph);
				for (String adjacent : g.getAdjacentVertices(vertex)) {
					...
					int weight = weights.containsKey(vertex) ? weights.get(vertex) : 1;
					
					if (prev[ix] == null) || finalweights[ix] < finalweights[previx] + weight) {
