...
public class DijkstraPanel extends JPanel implements Runnable{
	...
	public static final int START_X =  25, START_Y =  25,
							END_X   = 49, END_Y   = 49;
	
	private int[][] walls = new int[GRID_SIZE][GRID_SIZE];
	private	int[][] costs = new int[GRID_SIZE][GRID_SIZE];
	...
	private void DijkstraStep() {
		...
		for (Point p : getNeighbors(current)) {
			int altcost = costs[current.x][current.y] + walls[p.x][p.y];
			if (altcost < costs[p.x][p.y]) {
				from[p.x][p.y] = current;
				costs[p.x][p.y] = altcost;
	...
	private Point getLeast(ArrayList<Point> points) {
		...
		Point ret = null;
		for (Point p : points) {
			if (costs[p.x][p.y] < leastCost) {
				leastCost = costs[p.x][p.y];
				ret = p;
			}
		}
		return ret;
	...
	public void addNotify() {
		super.addNotify();
		System.out.println("DijkstraPanel starts Thread");
		mainThread = new Thread(this);
		mainThread.start();
	}
	
	public static void main(String[] args) {
		JFrame f = new JFrame("Dijkstra Implementation");
		DijkstraPanel main = new DijkstraPanel();
